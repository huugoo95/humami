...
## Working branch
1. Inspect `git status`, current branch and `git worktree list`. Preserve unrelated work. Fetch the intended remote when possible and report stale references if it fails. Read [branch naming](../../../docs/branching.md).
2. Retain the valid branch for this task. Otherwise create the appropriate branch before editing: feature/fix/docs/chore from updated develop; hotfix from master; release preparation from develop. Features use `feat/<spec-id>-<short-name>`, never an automatic replacement prefix. Do not silently switch a directory used by another active worker; isolate concurrent implementations explicitly.
...
3. Use an available authenticated GitHub connector or CLI to create/update the PR. Feature/fix/docs/chore target develop. Release targets master then back-merges to develop; hotfix targets master then develop. Include numeric ID/OpenSpec link, scope, evidence, applicable TDD note, risks and rollback using the [PR template](../../../.github/pull_request_template.md). Do not claim a PR exists if tool/auth access is absent.
